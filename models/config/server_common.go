...
func InitServerCfg(cfg *ServerCommonConf) {
	...
	maxServiceUnavailableCount = cfg.MaxServiceUnavailableCount
}
...
type ServerCommonConf struct {
	...
	MaxServiceUnavailableCount uint64 `json:"max_service_unavailable_count"`
...
func GetDefaultServerConf() *ServerCommonConf {
	return &ServerCommonConf{
		...
		UserConnTimeout:            10,
		Custom503Page:              "",
		MaxServiceUnavailableCount: math.MaxUint64, // 初始化默认值
		EnableApi:                  false,
		ApiBaseUrl:                 "",
		ApiToken:                   "",
	}
...
func UnmarshalServerConfFromIni(defaultCfg *ServerCommonConf, content string) (cfg *ServerCommonConf, err error) {
	...
	if tmpStr, ok = conf.Get("common", "max_service_unavailable_count"); ok {
		v, errRet := strconv.ParseUint(tmpStr, 10, 64)
		if errRet != nil {
			err = fmt.Errorf("Parse conf error: max_service_unavailable_count is incorrect")
			return
		} else {
			cfg.MaxServiceUnavailableCount = v
		}
	}
